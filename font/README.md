...
The **GameBoy** folder contains multiple `.png` files that can be loaded into a Game Boy Studio for developing games for the Nintendo Game Boy. Each font has a regular (light) and a -dark (inverse) version ready for use on light or dark backgrounds. Right now only the standard Spectrum character set is represented so ASCII plus pound and copyright symbols.
